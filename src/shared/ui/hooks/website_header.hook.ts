...
import { cartRoute, dashboardRoute, loginRoute } from "@/core/routes/routeNames";
...
const useWebsiteHeaderHook = () => {
  ...
  const isAuthenticatedFromRedux = useSelector((state: RootState) => state.authState.isAuthenticated);
  ...
  const { data: userResponse, isLoading: isUserLoading } = useGetUserQuery(undefined, {
    skip: !isAuthenticated,
  });

  const apiUser = userResponse?.data || ((userResponse as any)?.firstName ? userResponse : null);
  ...
  let firstName = (apiUser as any)?.firstName || (storedUser as any)?.firstName || "";
  let lastName = (apiUser as any)?.lastName || (storedUser as any)?.lastName || "";
